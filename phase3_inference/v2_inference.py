    ...
    def parse_user_prompt(self, user_prompt):
        ...
        for pattern in page_patterns:
            ...
            for match in matches:
                ...
                for part in parts:
                    name = part.strip()
                    # Filter out common words that aren't page names
                    exclude_words = {'with', 'css', 'html', 'for', 'all', 'the', 'a', 'an', 'in', 'on', 'and', 'or', 'of', 'to', 'from', 'by', 'each', 'every'}
                    # Only add valid page names (2+ letters, no special chars)
                    if len(name) > 2 and name.isalpha() and name not in exclude_words:
  ...
  return (
    ...
    def generate_multi_file_project(self, user_prompt):
        ...
        if 'react' in file_types:
            ...
            # Note: App.jsx will be generated by generate_routing_file() at the end
